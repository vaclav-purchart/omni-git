<div align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/tests-741-brightgreen" alt="741 tests">
  </p>
</div>
...
Bundles land in `src-tauri/target/release/bundle/`. Builds are per-platform: Tauri does not
support cross-compiling to macOS or Windows from another host, so produce each platform's
bundle on that platform (CI runners are the easy way).

## Development

```bash
npm test         # vitest — 62 files, 741 tests
...
- The frontend never talks to git directly. It calls typed commands generated by
  ...
  regenerate them by building the Rust side rather than editing that file.
