...
`predev` / `prebuild` / `pretest` npm hooks create it when it is missing. It costs one extra
Rust compile the first time (~50s on a warm cargo registry) which shares its cache with the
build that follows; after that the hooks see the file and skip straight through.

Generating it means *running* a binary that links the whole tauri stack, so if that fails on
your platform the build says so and tells you what to do — the file is plain TypeScript with no
platform-specific content, so copying it from a machine where `npm run bindings` works is a
legitimate fix. `npm run bindings` always regenerates.
...
On macOS the `.dmg` step runs an AppleScript to prettify the Finder window, which needs
Automation permission for whatever is running the build (System Settings → Privacy & Security →
Automation). Without it the bundle fails with `Not authorized to send Apple events to Finder.
(-1743)` — *after* `omni-git.app` has already been built successfully. `CI=true npm run tauri
build` skips the cosmetic step and produces the `.dmg` anyway.
...
```
scripts/bindings.mjs  generates src/ipc/bindings.ts (run by the npm pre* hooks)
